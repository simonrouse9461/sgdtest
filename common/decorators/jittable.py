...
def jittable(cls: T) -> T:
    ...
    src = synthesize__init__(cls).source
    ...
    for param in inspect.signature(cls.__init__).parameters.values():
        if param.annotation.__module__ not in globals_dict:
    ...
    # Writing `cls` back to caller's `globals()`, so that nested classes are visible to `@jit.script`
